...
namespace DAL
{
    public class DU_UserInfo
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 U_Id,U_RoleType,U_DepType,U_Position from U_UserInfo");
            strSql.Append(" where ");
        ...
        public DataSet SelectAdmin(string where)
        {
            ...
            strSql.Append(" U_Birthday as 出生日期, U_IdCard as 身份证号,U_Address as 家庭住址,U_Telephone as 联系电话");
            strSql.Append(" from U_UserInfo");
            if (where != "")
            {
