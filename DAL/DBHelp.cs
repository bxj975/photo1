...
namespace DAL
{
    public class DBHelp
    {
        ...
        /// 获取连接数据库对象
        /// </summary>
        public static SqlConnection GetConnection    // 获取连接数据库对象
        {
            get
            {
                strConn = System.Configuration.ConfigurationManager.ConnectionStrings["strConn"].ConnectionString.ToString();
                if (connection == null)
                {
        ...
        public static SqlCommand Command(string strSql)  //创建SqlCommand对象 sql语句不带参数
        {
        ...
        public static SqlCommand Command(string strSql, params SqlParameter[] values)  //创建SqlCommand对象 sql语句带参数
        {
        ...
        public static SqlDataReader ExecuteReader(string strSql)   //返回前向只读的结果集对象 sql语句不带参数(查询)
        {
            SqlCommand cmd = Command(strSql);
            return cmd.ExecuteReader();
        }
        //返回前向只读的结果集对象 sql语句带参数(查询)
        public static SqlDataReader ExecuteReader(string strSql, params SqlParameter[] values) //返回前向只读的结果集对象 sql语句带参数(查询) 
        {
        ...
        public static object ExecuteScalar(string strSql, params SqlParameter[] values) //返回第一行第一列的值 sql语句带参数(查询)
        {
            SqlCommand cmd = Command(strSql, values);
            return cmd.ExecuteScalar();
        }
        ...
        //返回受影响的行数 sql语句不带参数(更新 添加 删除)
        public static int ExecuteNonQuery(string strSql, params SqlParameter[] values)  //返回受影响的行数 sql语句不带参数(更新 添加 删除)
        {
            SqlCommand cmd = Command(strSql, values);
            return cmd.ExecuteNonQuery();
        }
        //断开连接方式的结果集 sql语句不带参数
        public static DataTable DataTable(string strSql)   //断开连接方式的结果集 sql语句不带参数
        {
        ...
        //断开连接方式的结果集 sql语句带参数
        public static DataTable DataTable(string strSql, params SqlParameter[] values)//断开连接方式的结果集 sql语句带参数
        {
            DataSet ds = new DataSet();
            SqlDataAdapter adapter = new SqlDataAdapter(strSql, GetConnection);
        ...
        ///ProcName存储过程的名称
        public static SqlCommand ProcCommad(string ProcName) //使用存储过程的获得SQLCommand对象
        {
        ...
        ///ProcName存储过程的名称
        public static SqlCommand ProcCommad(string ProcName, SqlParameter[] param) //使用带参数存储过程的获得SQLCommand对象
        {
            SqlCommand comm = new SqlCommand(ProcName, GetConnection);
        ...
        //存储过程返回一个前向只读的结果集
        public static SqlDataReader ProExecuteReader(string ProName)//存储过程返回一个前向只读的结果集
        {
            SqlCommand comd = ProcCommad(ProName);
            return comd.ExecuteReader();
        }
        //带参数存储过程返回一个前向只读的结果集
        public static SqlDataReader ProExecuteReader(string ProName, SqlParameter[] param)//带参数存储过程返回一个前向只读的结果集
        {
            SqlCommand comd = ProcCommad(ProName, param);
            return comd.ExecuteReader();
        }
        //不带参数的存储过程的实现受影响行数
        public static int ProExecuteNonQuery(string ProName)//不带参数的存储过程的实现受影响行数
        {
            SqlCommand comd = ProcCommad(ProName);
            return comd.ExecuteNonQuery();
        }
        //带参数的存储过程的实现受影响行数
        public static int ProExecuteNonQuery(string ProName, SqlParameter[] param) //带参数的存储过程的实现受影响行数
        {
            SqlCommand comd = ProcCommad(ProName, param);
            return comd.ExecuteNonQuery();
        }
        ///使用带参数获得DataSet对象
        public static DataSet DateSet(string strSql, SqlParameter[] param)//使用带参数获得DataSet对象
        {
            DataSet ds = new DataSet();
            SqlDataAdapter adapter = new SqlDataAdapter(strSql, GetConnection);
        ...
        ///使用不带参数获得DataSet对象
        public static DataSet DateSet(string strSql)//使用不带参数获得DataSet对象
        {
            DataSet ds = new DataSet();
            SqlDataAdapter adapter = new SqlDataAdapter(strSql, GetConnection);
            adapter.Fill(ds);
        ...
        public static DataSet ExecuteDataSet(string cmdText, params SqlParameter[] commandParameters)// 返回一个DataSet数据集
        {
        ...
        public static DataSet getAllBookType()  // 获得所有类型
        {
            try
            {
                string strSql = "select * from CustomType";
